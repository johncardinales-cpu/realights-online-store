...
export function Storefront({ page }: { page: 'home' | 'catalog' | 'checkout' }) {
  ...
  const shown = active.filter((p) => (category === 'All' || p.category === category) && [p.sku, p.name, p.category, p.specs].join(' ').toLowerCase().includes(query.toLowerCase()));
  ...
  const ProductCard = ({ product }: { product: Product }) => (
    <article className="product">
      ...
      <div className="body">
        <div className="chips"><span className="badge green">{product.category}</span><span className="badge gold">{product.stock > 0 ? `${product.stock} available` : 'Quote availability'}</span></div>
        <h3>{product.name}</h3>
  ...
  return <>
    ...
    <main>
      {page === 'home' && <>
        <section className="hero"><div className="wrap heroGrid"><div className="heroMain"><div><span className="eyebrow">Customer-facing store</span><h1>Solar products ready for quotes.</h1><p className="lead">A separate public storefront for Realights. Customers browse products, send inquiries, and submit quote requests. Backoffice stays separate.</p><div className="actions"><a className="btn primary" href="/catalog">Shop catalog</a><a className="btn" href="#lead">Ask for quote</a></div></div><div className="stats"><div className="stat"><strong>{active.length}</strong><span>products</span></div><div className="stat"><strong>{active.reduce((s,p)=>s+p.stock,0)}</strong><span>available stock</span></div><div className="stat"><strong>RFQ</strong><span>quote-first checkout</span></div><div className="stat"><strong>Safe</strong><span>manual import</span></div></div></div><div className="card media"><div><img src="https://www.bluesunpv.com/wp-content/uploads/2024/10/hm-product-1.png" alt="Solar products"/><h2>Ready for ads and leads</h2><p>Facebook, Instagram, TikTok, Google, Messenger and direct inquiries can be tracked.</p></div></div></div></section>
        <section className="section"><div className="wrap head"><div><h2>Featured products</h2><p className="lead">Selected products from the current store catalog.</p></div><span className="pill">Customer view only</span></div><div className="wrap grid">{active.slice(0,6).map((p) => <ProductCard product={p} key={p.sku} />)}</div></section>
        <LeadForm onSubmit={submitLead} success={success} />
      </>}
      {page === 'catalog' && <section className="section"><div className="wrap head"><div><h2>Catalog</h2><p className="lead">Products shown from the latest manual inventory setup.</p></div><span className="pill">{shown.length} products</span></div><div className="wrap toolbar"><div className="links">{categories.map((c) => <button className={c===category?'primary':''} key={c} onClick={()=>setCategory(c)}>{c}</button>)}</div><input className="input search" placeholder="Search products" value={query} onChange={(e)=>setQuery(e.target.value)} /></div><div className="wrap grid">{shown.map((p)=><ProductCard product={p} key={p.sku}/>)}</div></section>}
      {page === 'checkout' && <Checkout lines={lines} onSubmit={submitOrder} success={success} />}
    </main>
    {drawer && <aside className="drawer"><div className="head"><h2>Cart</h2><button className="btn" onClick={()=>setDrawer(false)}>Close</button></div>{lines.length ? lines.map((line)=><div className="cartItem" key={line.sku}><img src={line.product.image} alt=""/><div><strong>{line.product.name}</strong><p className="summary">{line.sku}</p><div className="qty"><button onClick={()=>qty(line.sku,line.qty-1)}>-</button><span>{line.qty}</span><button onClick={()=>qty(line.sku,line.qty+1)}>+</button></div></div><button onClick={()=>qty(line.sku,0)}>Remove</button></div>) : <div className="empty">Cart is empty.</div>}<a className="btn primary" href="/checkout">Proceed to checkout</a></aside>}
  </>;
}
