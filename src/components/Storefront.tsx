...
export function Storefront({ page }: { page: 'home' | 'catalog' | 'checkout' }) {
  ...
  const shown = active.filter((p) => (category === 'All' || p.category === category) && [p.sku, p.name, p.category, p.specs, p.brand, p.series].join(' ').toLowerCase().includes(query.toLowerCase()));
  ...
  const ProductCard = ({ product }: { product: Product }) => (
    <article className="product">
      ...
      <div className="body">
        <div className="chips"><span className="badge green">Realights</span><span className="badge">{product.brand || 'Supplier brand'}</span><span className="badge gold">{product.stock > 0 ? `${product.stock} available` : 'Quote availability'}</span></div>
  ...
  return <>
    ...
    <main>
      {page === 'home' && <>
        <section className="hero"><div className="wrap heroGrid"><div className="heroMain"><div><span className="eyebrow">Realights solar store</span><h1>Clean energy products, ready for quotation.</h1><p className="lead">A separate public storefront for Realights with a curated supplier catalog, quote-first checkout, social media lead capture, and a separated backoffice.</p><div className="actions"><a className="btn primary" href="/catalog">Shop catalog</a><a className="btn" href="#lead">Ask for quote</a></div></div><div className="stats"><div className="stat"><strong>{active.length}</strong><span>products</span></div><div className="stat"><strong>{active.reduce((s,p)=>s+p.stock,0)}</strong><span>available stock</span></div><div className="stat"><strong>RFQ</strong><span>quote-first checkout</span></div><div className="stat"><strong>Safe</strong><span>manual import</span></div></div></div><div className="card media"><div><img src="https://www.bluesunpv.com/wp-content/uploads/2024/10/hm-product-1.png" alt="Solar products"/><h2>Realights + Bluesun catalog</h2><p>Bluesun product names, series, images, and specification wording are presented as a featured supplier catalog for Realights quotation workflows.</p></div></div></div></section>
        <SupplierSection />
        <section className="section"><div className="wrap head"><div><h2>Featured Bluesun products</h2><p className="lead">Selected supplier products displayed with Realights quote-first ordering.</p></div><span className="pill">Customer view only</span></div><div className="wrap grid">{active.slice(0,6).map((p) => <ProductCard product={p} key={p.sku} />)}</div></section>
        <LeadForm onSubmit={submitLead} success={success} />
      </>}
      {page === 'catalog' && <section className="section"><div className="wrap head"><div><h2>Catalog</h2><p className="lead">Realights customer catalog with supplier brand, series, product name, and key specifications.</p></div><span className="pill">{shown.length} products</span></div><div className="wrap toolbar"><div className="links">{categories.map((c) => <button className={c===category?'primary':''} key={c} onClick={()=>setCategory(c)}>{c}</button>)}</div><input className="input search" placeholder="Search products, SKU, brand, series" value={query} onChange={(e)=>setQuery(e.target.value)} /></div><div className="wrap grid">{shown.map((p)=><ProductCard product={p} key={p.sku}/>)}</div></section>}
      {page === 'checkout' && <Checkout lines={lines} onSubmit={submitOrder} success={success} />}
    </main>
    {drawer && <aside className="drawer"><div className="head"><h2>Cart</h2><button className="btn" onClick={()=>setDrawer(false)}>Close</button></div>{lines.length ? lines.map((line)=><div className="cartItem" key={line.sku}><img src={line.product.image} alt=""/><div><strong>{line.product.name}</strong><p className="summary">{line.product.brand || 'Supplier'} • {line.product.sku}</p><div className="qty"><button onClick={()=>qty(line.sku,line.qty-1)}>-</button><span>{line.qty}</span><button onClick={()=>qty(line.sku,line.qty+1)}>+</button></div></div><button onClick={()=>qty(line.sku,0)}>Remove</button></div>) : <div className="empty">Cart is empty.</div>}<a className="btn primary" href="/checkout">Proceed to checkout</a></aside>}
  </>;
}

function SupplierSection() {
  return <section className="section"><div className="wrap card" style={{ padding: 24 }}><div className="head"><div><span className="eyebrow">Featured supplier</span><h2>Bluesun Solar product catalog</h2><p className="lead">Realights can present Bluesun product titles, product categories, series names, images, and key specifications in a clean quote-based store layout.</p></div><div className="stat"><strong>Bluesun</strong><span>supplier catalog badge</span></div></div><div className="grid4"><div className="stat"><strong>190+</strong><span>countries exported</span></div><div className="stat"><strong>12GW+</strong><span>PV shipped</span></div><div className="stat"><strong>3GWh</strong><span>BESS shipped</span></div><div className="stat"><strong>10+</strong><span>overseas branches</span></div></div><p className="summary" style={{ marginTop: 14 }}>Realights remains the store brand. Bluesun appears as a featured product supplier/brand source until your final dealership, logo permission, pricing, and inventory files are approved.</p></div></section>;
}
